...
class Core_Controller extends MX_Controller {
     ...
     */
    public function sendError($errorCode, $data = '', $status = RestStatus::STATUS_PHP_ERROR,
     ...
     */
    public function viewRender($data, $view, $navInfo)
    {
        $req = array(
            'ua' => $_SERVER['HTTP_USER_AGENT'],
        );

        $templateHost = 'Host:' . CodeConst::VIEW_TEMPLATE_DOMAIN;
        $templateConf = CodeConst::getViewTemplateConf();
        $url = "http://{$templateConf['ip']}:{$templateConf['port']}/$view";
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array($templateHost));
        curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "POST");
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE));
    ...
    /**
     * 与前端node实现模板html，代替view功能(错误页面)
     */
    public function viewErrorRender($data = array(), $view = CRMConst::PAGE_500_VIEW)
    {
        $this->viewRender($data, $view, null,'errors');
    }

    /**
     * 与前端node实现模板html，代替view功能(错误页面)
     */
    public function viewNoPermissionRender($data = array(), $view = CRMConst::PAGE_NO_PERMISSION_VIEW)
    {
        $this->viewRender($data, $view, null,'errors');
    }
